<template>
  <div id="modify">
    ...
    <div class="Crumbs">
      ...
      <span @click="Uid">{{ this.$route.query.Uname }}</span> &nbsp;>&nbsp;
      <span @click="UUid">{{ this.$route.query.UUname }}</span> &nbsp;>&nbsp;
      <span @click="UUUid">{{ this.$route.query.UUUname }}</span> &nbsp;>&nbsp;
      <span @click="Suntask" v-for="(item, index) in task" :key="index">{{
        item.taskname
      }}</span>
    </div>
    <div class="Alltask" v-for="(val, index) in task" :key="index">
      <div
    ...
    Detailed(id, index) {
      ...
      this.$router.push({
        ...
        query: {
          ...
          UUname: this.$route.query.UUname,
          UUUid: this.$route.query.UUUid,
          UUUname: this.$route.query.UUUname,
          UUUUid: this.task[0]._id,
          UUUUname: this.task[0].taskname,
        },
      });
    },
    HomePage() {
